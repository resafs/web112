...
class Navigation_admin:
    ...
    def show_window_admin(self):
        ...
        btn_kasir = Button(frame,text='Kasir', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('kasir'))
        btn_kasir.pack(padx=4,pady=5)
        btn_kelola = Button(frame,text='Tentang Menu', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('menu'))
        btn_kelola.pack(padx=4,pady=5)
        btn_keuntungan = Button(frame,text='Keuntungan', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('keuntungan'))
        btn_keuntungan.pack(padx=4,pady=5)
        btn_terjual = Button(frame,text='Terjual', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('penjualan'))
        btn_terjual.pack(padx=4,pady=5)
        
        btn_kelola = Button(frame,text='Melihat user', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('lihat user'))
        btn_kelola.pack(padx=4,pady=5)
        btn_keuntungan = Button(frame,text='Mendaftarkan user', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('daftar user'))
        btn_keuntungan.pack(padx=4,pady=5)
        btn_terjual = Button(frame,text='Menghapus', width=15, height=2,bg='#8fffff',fg='#000',command= lambda: self.quiz('hapus user'))
        btn_terjual.pack(padx=4,pady=5)
    ...
    def quiz(self, nav):
        ...
        if not quiz:
            ...
            btn_oke = Button(self.root, text='Masuk', font=('arial',13,'bold'), command = lambda: self.get_in(s_pass.get(), nav, self.root) , width = 8)
    ...
    def get_in(self, s_pass, nav, window):
        global quiz
...
class Regis:
    ...
    def show_window_regis(self):
        ...
        btn_oke = Button(self.window, text='Kembali', font=('arial',13,'bold'), command = lambda: self.dstry(self.window), width = 20)
...
class Untung:
    ...
    def show_window_untung(self):
        ...
        tanggal = StringVar(self.window)
        tanggal.set("Pilih tanggal")

        hari = Label(self.window,text='Tanggal:',font=('arial',15,'bold'))
        hari.place(x=6,y=20)
        
        droplist_day = self.transac.get_day_keuntungan()
        entry_kode = OptionMenu(self.window,tanggal,*droplist_day)
        entry_kode.config(highlightbackground = "#b3b3b3", highlightthickness=1,width=14)
        entry_kode.place(x=95,y=24)

        btn_cek = Button(self.window,text='Cek',command= lambda: self.transac.get_untung_today(tanggal.get()),width=18)
...
class Jual:
    ...
    def show_window_jual(self):
        ...
        btn_cek = Button(self.window,text='Cek',command= lambda: self.show_penjualan(tanggal.get()),width=18)
        btn_cek.place(x=86,y=75)
